...
# Before you start, don't forget to set your working directory  if you need to.
...
# as learning about the most commonly used dplyr functions (select, filter,
# mutate, arrange, _joins, _binds, group_by and summarise).
# However, these functions are most useful when we have "tidy data". In this
# workshop, we're going to learn about tidy data: what it is, why it's important
# and how to take "untidy data" and manipulate it into a tidy format that we can
# use for data analysis.

# Let's load in our packages
# A reminder that tidyverse contains a suite of packages such as dplyr,
# ggplot2 (for plotting), stringr (dealing with character strings).
...
#  This is easier to see in practice, so let's look at some data.
# We're going to use some sample data included in the tidyr pacakge.
...
# For example, table2 suffers from being too long, and has individual cells that
# aren't values of an observation, but specify the variable that the value in
# the adjacent cell should be applied to.
...
# Note that separate will automatically find a character to separate on if you
# don't specify the sep argument (it will try to find a non-alpha-numeric
# character). However, it's better practice to be explicit about what to
# separate on. We can see that table3 now looks tidy, and satisfies the three
# principles of tidy data.

# However, if we look at our new dataset, tidy3, we can see that cases and
# population have been created as character variables. THis is signified by the
# "<chr>" below the variable name. This is not what we wanted as this will limit
# our ability to conduct analysis on what should be numeric variables. There
# are two ways we could fix this: either we can change the variable type
# after-the-fact, OR we can specify that the separate function should convert
# the data type for the new variables.

# Option 1, using "as" functions to change variable type after-the-fact
tidy3$cases <- as.integer(tidy3$cases)
tidy3$population <- as.integer(tidy3$population)

print(tidy3)

# we can see that this has worked by seeing the variable type under the variable
# name, and it lets us do some arithmetic on the data.
sum(tidy3$cases)
mean(tidy3$population)

# Option 2, allowing the separate function to assign new variable types.
tidy3 <- table3 %>% 
  tidyr::separate(rate,
...
# that this isn't always a good idea, as R can make some decisions that we
...
# in MOST circumstances. We can easily pull these back together
...
# Table2 is now tidy, with half as many rows, and each row representing a
...
# table4a contains the number of cases in a country in each year. It suffers
# from a very common problem for tables that have been designed to be "human
# readable" instead of "machine readable" - that years are presented as multiple
# columns instead of making "year" a single variable.
# We can fix this with the pivot_longer function.
# pivot_longer is the inverse of pivot_wider, as it takes a dataframe that is
# too wide and makes it longer. ie, it can take a dataset where a single row
# represents multiple observations, and turn it into one where a row
...
# Another common problem you will encounter with input data is the problem of
# missing data. Missing data is information that your dataset SHOULD have, but
# doesn't. However, it's important to be aware of the fact that there are two
...
# We have an EXPLICIT missing value for the return value of quarter 4 of 2015.
# This is indicated by the "NA" value in the data frame. We should know the
# return from Q4 of 2015, but we don't.
# However we also have an IMPLICIT missing value for the return value of Quarter
# 1 of 2016. It's implicitly missing because there isn't any observation marked
# for it. The entire row is absent from the dataframe (not just marked as NA).
...
# in the data frame).
# We can pipe this pivot_wider() function back into a pivot_longer() function
# to give us our original data frame, but with our explicit missing data now
# included.
...
# QUICK challenge: how could you sort the dataframe so it ran in reverse
# chronology, with the most recent quarter first?
# (hint, think back to the first workshop)

# Replacing Specific Values ----

# Now let's say we investigated our source data, and we found out that an NA was
# inputted wherever the return was 0. This is a common error that can happen
# when taking data from one format to another, or from one software to another.
...
# Alternatively it might be the case that 0s (or some other value) have been
# loaded in where we actually don't know what the value should be. ie we should
# have NAs but don't. We can assign NAs in the same method as above
...
# Another reason we could have NAs or blank entries in our data is if blank/NA
# values are supposed to be considered to be the same as in the cell above them.
# This is another common problem in data that has been designed to be human
# readable instead of machine readable, or from transferring between software 
# that have different ways optimising memory size of data.
# Consider the below example:

table6 <- read_csv("data/table6.csv")
print(table6)

# Table6 is almost identical to table1. The only difference is that there are NA
# values in the country column. However, it is obvious looking at it that these
# NA values should in fact just carry the value of the nearest cell above that
# has a value inputted.
...
# Character strings often require more tidying than numeric data, as there is
# more variability in how they could be inputted by manual data input.
# For manipualting character strings, we'll be using the stringr package, which
# is part of the tidyverse suite of packages.
...
# some have lower case letters, some have spaces.
...
# Note that the str_replace function would behave similarly BUT would only
# remove the first instance of a " " in a string so would not be useful if any
# of the strings have white space at the end of the variable, or had a double
# space.
...
# Another problem with this data frame is that the names of the variables are
# inconsistent in how they use spacing and how they capitalise variables. It can
# also be quite annoying to work with variable names that have spaces in them,
# as this can often create errors in code if we try to call variable names
# directly in other functions. We could specify new names for our variables
# using the dplyr function rename(), OR we could use the janitor function
# clean_names() to change all of the names in a dataframe.
kebabs <- janitor::clean_names(kebabs, case = "snake")
print(kebabs)

# The "case" argument allows us to specify different standard ways of
# capitalising letters and/or dealing with spaces. Some R users will tell you
# that variable names should ALWAYS be lower case, with no spaces (sometimes
# with underscores as spaces). I'm generally of the opinion that lower case
# variable names are preferable, but there are some reasons you might capitalise
# variable names, such as to distinguish acronyms, or you can capitalise the
# names of variables which have been "manipulated-in", while leaving the
# "original" variables lower-case.
# Different teams/users will have different rules and conventions for this, so 
# be sure you are aligned with your team.

# Dates and Times ----

# Another tricky variable type is data that represents dates or times.

# In some circumstances, it will be appropriate to just keep data that specifies
# times in a numeric format. For example, above we just kept the year variable
# as an integer. However, it's often the case that we have day-specific, and
# even time specific data, and keeping this data in an appropriate format is
# important for our analysis. R is able to hold information in the format
# <date>, <time>, or <dttm> (date-time), and these variables are often
...
# We can see when we print the dataframe that the Date variable is actually
# being saved in R as a character variable. To turn it into a date format, we
# will need to specify it explicitly.

# There are a family of functions that can take character strings and turn them
# into date or time variables. Below we use the function (dmy) because our data
# is structured as "Day"/"Month"/"Year", but other functions in lubridate can
# accommodate a range of different structures for date and time (eg mdy).

accidents$Date <- lubridate::dmy(accidents$Date)
print(accidents)

# You might get a warning message after running this saying that R is unable to
# identify the current timezone. If this is the case, we can manually set the
# time zone with the following function. This is relatively unimportant unless
# we want to compare dates/times in our dataset with our current date/time.

Sys.setenv(TZ='UTC')

# The fact that R now recongises this as a date variable means that even though
# it appears as a recognisable character string for us, under the hood, R has
# saved it as a numeric variable.
...
# Challenge ----
# Use the above method to think of other ways to group when accidents happen.
# Note: you can do this in multiple ways. Think about how you might
# group by time of day, rush hour times, group by daytime/night-time etc.
# Have a google to search for any useful functions or bits of code
# to do what you want to do.
# Think about interesting insights you can get out of just the date of accidents
...
# First, make it a numeric variable
...
# Assuming that our input was the same each time, we could always use this
# same chunk of code to produce this same tidied output.
...
# 1) We talked here about how and why you should make your data tidy. However,
# there are legitimate reasons why you might keep your data in a "non-tidy" way.
# This could be for performing specific tasks, for exporting human-readable
...
# 3) This chapter from an intro-to-R online workbook is useful for understanding
# tidy data in some more detail:
# https://robwschlegel.github.io/Intro_R_Workshop/tidy.html 
